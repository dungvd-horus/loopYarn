...
half _EmissiveAnimationSpeed;
half _EmissiveAnimationIntensity;
half _EmissiveAnimationMin;
...
sampler2D _TwoHalfColorsMaskTex;
half4 _TwoHalfColorsMaskTex_ST;
half4 _TwoHalfColorsColor1;
half4 _TwoHalfColorsColor2;
half _TwoHalfColorsThreshold;
half _TwoHalfColorsSmoothness;
CBUFFER_END
...
half4 frag (v2f i) : COLOR {
	...
	#if TWOHALFCOLORS
		// Apply TwoHalfColors effect to col_base before lighting calculations
		// Sample the mask texture using the main UV coordinates
		half maskValue = tex2D(_TwoHalfColorsMaskTex, TRANSFORM_TEX(uv, _TwoHalfColorsMaskTex)).r;
		
		half4 modifiedBase = col_base;
		
		#if _TWOHALFCOLORSMODE_REPLACE
			// Replace mode: fully replace the base color with specified colors based on mask
			// If mask value is at or below threshold, use Color1; if above threshold, use Color2
			half smoothness = max(_TwoHalfColorsSmoothness, 0.001); // Avoid division by zero
			half transitionFactor = saturate((maskValue - _TwoHalfColorsThreshold + smoothness) / (smoothness * 2.0));
			half4 color1 = half4(_TwoHalfColorsColor1.rgb, col_base.a);
			half4 color2 = half4(_TwoHalfColorsColor2.rgb, col_base.a);
			modifiedBase = lerp(color1, color2, transitionFactor);
		#else  // _TWOHALFCOLORSMODE_MULTIPLY
			// Multiply mode: multiply the existing base color by specified colors based on mask
			half smoothness = max(_TwoHalfColorsSmoothness, 0.001); // Avoid division by zero
			half transitionFactor = saturate((maskValue - _TwoHalfColorsThreshold + smoothness) / (smoothness * 2.0));
			half4 color1 = half4(col_base.rgb * _TwoHalfColorsColor1.rgb, col_base.a);
			half4 color2 = half4(col_base.rgb * _TwoHalfColorsColor2.rgb, col_base.a);
			modifiedBase = lerp(color1, color2, transitionFactor);
		#endif
		
		// Apply the modified base color
		col_base = modifiedBase;
	#endif
	...
	#endif
	#if EMISSIVE_ANIMATED
		// Calculate animated emissive intensity using sine wave
		half emissiveIntensity = _EmissiveAnimationMin + (1 - _EmissiveAnimationMin) * 
			(0.5 + 0.5 * sin(_Time.y * _EmissiveAnimationSpeed) * _EmissiveAnimationIntensity);
		col_emissive *= emissiveIntensity;
	#endif
	#if META
