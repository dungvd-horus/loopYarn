    ...
    public class RopeMesh : MonoBehaviour
    {
        ...
        public void SetColor(Color _color)
        {
            if (meshRenderer != null)
            {
                ...
                // block.SetColor(ShaderPropertiesLib.Color, _color);
        ...
        {
            if (RopeLogic.EndPoint == null) return;
            if (block == null)
            {
                block = new MaterialPropertyBlock();
                meshRenderer.GetPropertyBlock(block);
            }
            float currentDist = Vector3.Distance(StartPoint.position, RopeLogic.EndPoint.position);
            float distRatio = currentDist / ThresholdDistance;
            float rsOffsetY = distRatio * ThresholdOffsetY;
            block.SetVector("_TwoHalfColorsMaskTex_ST", new Vector4(1f, 1f, 0f, rsOffsetY));
            meshRenderer.SetPropertyBlock(block);
        }

        // Debug methods - can be called from Inspector via custom editor or runtime code
