...
public class LevelCollectorsSystem : MonoBehaviour
{
    ...
    private void InitializeCollections()
    {
        ...
        CurrentLocks = CurrentLocks ?? new List<LockObject>();
        CurrentCollectors = CurrentCollectors ?? new List<ColorPixelsCollectorObject>();
        ObjectsInColumns = ObjectsInColumns ?? new List<CollectorColumn>();
        CollectorControllersColumns = CollectorControllersColumns ?? new List<List<CollectorController>>();
        _tempProgressedIndex = _tempProgressedIndex ?? new List<int>();
        _tempCurrentGroup = _tempCurrentGroup ?? new List<ColorPixelsCollectorObject>();
        _collectorIndexByID = _collectorIndexByID ?? new Dictionary<int, int>();
        _collectorsByID = _collectorsByID ?? new Dictionary<int, ColorPixelsCollectorObject>();
